...
null_stmt :  //+++            // NULL statement (no operator)
       T_NULL
     ;
...
create_table_stmt : //+++
      T_CREATE T_EXTERNAL T_TABLE (T_IF T_NOT T_EXISTS)? table_name create_table_preoptions? create_table_definition delm location
     ;
delm : T_ROW  T_FORMAT T_DELIMITED T_FIELDS T_TERMINATED T_BY L_CHAR ;
location : T_LOCATION L_FILE ;
...
create_function_stmt : //+++
      //(T_ALTER | T_CREATE (T_OR T_REPLACE)? | T_REPLACE)? T_FUNCTION ident create_routine_params? create_function_return (T_AS | T_IS)? declare_block_inplace? single_block_stmt
       // T_CREATE T_FUNCTION
       dtype ident T_OPEN_P fanc_param_elem? T_CLOSE_P T_OPEN_B elems* T_CLOSE_B
    ;
  //func_body: dtype ident T_OPEN_P fanc_params? T_CLOSE_P T_OPEN_B fanc_body T_CLOSE_B  ;
  fanc_param_elem: (dtype ident ((T_COMMA dtype ident)*)?);
  //fanc_body : elems*;
  elems: creat_return | if_in_func  | for_in_func | assignment_in_func | stmt | scope;
  scope: T_OPEN_B elems* T_CLOSE_B;
  if_in_func: T_IF T_OPEN_P bool_expr T_CLOSE_P T_OPEN_B elems* T_CLOSE_B if_rest?  ;
  if_rest: (else_if*)? elsee?;
  else_if: T_ELSE T_IF T_OPEN_P bool_expr T_CLOSE_P T_OPEN_B elems* T_CLOSE_B ;
  elsee: T_ELSE T_OPEN_B elems* T_CLOSE_B ;
  for_in_func: T_FOR T_OPEN_P for_params T_SEMICOLON for_params T_SEMICOLON for_params T_CLOSE_P T_OPEN_B elems* T_CLOSE_B ;
  for_params: for_param ;
  for_param: ((bool_expr| assignment_0) (T_COMMA(bool_expr| assignment_0))*)?;
  creat_return: T_RETURN (expr|select_stmt) T_SEMICOLON;
  assignment_in_func: assignment_0 T_SEMICOLON ;
  assignment_0:(declare|assignment_1|assignment_2);
  //func_call:ident T_OPEN_P (ident (T_COMMA ident)*  )?;
  declare : dtype ident declare_rest?(T_COMMA ident declare_rest?)* ;
  declare_rest: T_EQUAL (expr|select_stmt);
  assignment_1:(T_SUB T_SUB|T_ADD T_ADD)? ident;
  assignment_2:ident (assingnment_rest|T_SUB T_SUB|T_ADD T_ADD)?;
  assingnment_rest: (T_EQUAL|T_ADD T_EQUAL|T_SUB T_EQUAL |T_DIV T_EQUAL |T_MUL T_EQUAL) (expr|select_stmt);
  //assignment_3: T_EQUAL expr;
...
bool_expr :                               // Boolean condition
     ...
     | bool_expr_atom
     | NOT? T_OPEN_P bool_expr T_CLOSE_P

 ;
//missing_close_Pracite :  ;
bool_expr_atom : //+++
      bool_expr_unary
    | bool_expr_binary
    | expr
...
T_DELIMITER       : D E L  I M I T E R  ;            //'Delimiter'  -> pushMode(INSIDE);
T_DESC            : D E S C ;
